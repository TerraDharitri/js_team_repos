...
describe("test smart contract interactor", function () {
    ...
    it("should set transfers (payments) on contract calls (transfer and execute)", async function () {
        ...
        assert.equal(transaction.data.toString(), `DCDTTransfer@${hexFoo}@0a@${hexDummyFunction}`);
        ...
        transaction = new Interaction(contract, dummyFunction, [])
            .withSender(alice)
            .withMultiDCDTNFTTransfer([TokenFoo(3), TokenBar(3140)])
            .buildTransaction();
    ...
    it("should interact with 'lottery-dcdt'", async function () {
        this.timeout(30000);
        let abi = await loadAbiRegistry("src/testdata/lottery-dcdt.abi.json");
        let contract = new SmartContract({ address: dummyAddress, abi: abi });
        let controller = new SmartContractController({ chainID: "D", networkProvider: provider, abi: abi });
