...
export class DataApiService {
  ...
  public async getDcdtTokenPrice(identifier: string, timestamp?: number): Promise<number | undefined> {
    return await this.cachingService.getOrSet(
      ...
      async () => await this.getDcdtTokenPriceRaw(identifier, timestamp),
      CacheInfo.DataApiTokenPrice(identifier, timestamp).ttl
    );
  }

  private async getDcdtTokenPriceRaw(identifier: string, timestamp?: number): Promise<number | undefined> {
