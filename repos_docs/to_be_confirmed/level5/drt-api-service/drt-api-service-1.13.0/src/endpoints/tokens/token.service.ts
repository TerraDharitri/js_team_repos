...
export class TokenService {
  ...
  async getFilteredTokens(filter: TokenFilter): Promise<TokenDetailed[]> {
    ...
    if (filter.includeMetaDCDT !== true) {
      tokens = tokens.filter(token => token.type === TokenType.FungibleDCDT);
    }
  ...
  async getTokensForAddress(address: string, queryPagination: QueryPagination, filter: TokenFilter): Promise<TokenWithBalance[]> {
    ...
    for (const token of tokens) {
      if (token.type === TokenType.MetaDCDT) {
  ...
  async getTokenForAddress(address: string, identifier: string): Promise<TokenDetailedWithBalance | undefined> {
    ...
    if (TokenUtils.isNft(identifier)) {
      ...
      tokenWithBalance = new TokenDetailedWithBalance({ ...token, ...dcdtData });
    }
  ...
  async getAllTokensForAddress(address: string, filter: TokenFilter): Promise<TokenWithBalance[]> {
    ...
    for (const tokenIdentifier of Object.keys(dcdts)) {
      ...
      if (dcdt.type && this.nftSubTypes.includes(dcdt.type)) {
        switch (dcdt.type as NftSubType) {
          case NftSubType.DynamicNonFungibleDCDT:
          case NftSubType.NonFungibleDCDTv2:
            dcdt.type = NftSubType.NonFungibleDCDT;
            dcdt.subType = dcdt.type;
            break;
          case NftSubType.DynamicMetaDCDT:
            dcdt.type = NftType.MetaDCDT;
            dcdt.subType = NftSubType.DynamicMetaDCDT;
            break;
          case NftSubType.DynamicSemiFungibleDCDT:
            dcdt.type = NftType.SemiFungibleDCDT;
            dcdt.subType = NftSubType.DynamicSemiFungibleDCDT;
            break;
          default:
            dcdt.subType = NftSubType.None;
            break;
        }
      }
      ...
      if (dcdt.type === '') { // empty type can come from gateway
  ...
  async getTokenAccounts(pagination: QueryPagination, identifier: string): Promise<TokenAccount[] | undefined> {
    ...
    for (const tokenAccount of tokenAccounts) {
      result.push(new TokenAccount({
        ...
        identifier: tokenAccount.type === TokenType.MetaDCDT ? tokenAccount.identifier : undefined,
      }));
    }
      ...
      if (token) {
        ...
        if (item.roles) {
          ...
          resultItem.role = new TokenRoles({
            canLocalMint: token.type === TokenType.FungibleDCDT ? addressRoles.includes('DCDTRoleLocalMint') : undefined,
            canLocalBurn: token.type === TokenType.FungibleDCDT ? addressRoles.includes('DCDTRoleLocalBurn') : undefined,
            canAddQuantity: token.type === TokenType.MetaDCDT ? addressRoles.includes('DCDTRoleNFTAddQuantity') : undefined,
            canAddUri: token.type === TokenType.MetaDCDT ? addressRoles.includes('DCDTRoleNFTAddURI') : undefined,
            canCreate: token.type === TokenType.MetaDCDT ? addressRoles.includes('DCDTRoleNFTCreate') : undefined,
            canBurn: token.type === TokenType.MetaDCDT ? addressRoles.includes('DCDTRoleNFTBurn') : undefined,
            canUpdateAttributes: token.type === TokenType.MetaDCDT ? addressRoles.includes('DCDTRoleNFTUpdateAttributes') : undefined,
  ...
  async getAllTokensRaw(): Promise<TokenDetailed[]> {
    ...
    for (const token of tokens) {
      ...
      token.type = TokenType.FungibleDCDT;
    }
  ...
  private async getDcdtAccountsCount(identifier: string): Promise<number | undefined> {
    try {
