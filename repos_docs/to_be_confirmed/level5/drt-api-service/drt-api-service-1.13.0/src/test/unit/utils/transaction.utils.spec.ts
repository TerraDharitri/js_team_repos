...
describe('Transaction Utils', () => {
  it('tryExtractCollectionIdentifierFromChangeSftToMetaDcdTransaction', () => {
    ...
    transaction = new ShardTransaction();
    transaction.data = 'Y2hhbmdlU0ZUVG9NZXRhRENEVA==';
    tryExtractSftChange = new SftChangeTransactionExtractor;
    expect(tryExtractSftChange.extract(transaction)).toBeUndefined();

    transaction = new ShardTransaction();
    transaction.data = 'Y2hhbmdlU0ZUVG9NZXRhRENEVEA1MjQ1NTc0MTRkNGY0MTQ2MmQzNjM3NjE2MzM0MzlAMTJAOWFhNjNj';
    tryExtractSftChange = new SftChangeTransactionExtractor();
  ...
  it('tryExtractNftMetadataFromNftCreateTransaction from transaction', () => {
    ...
    transaction = new ShardTransaction();
    transaction.data = '';
  ...
  it('tryExtractNftMetadataFromNftCreateTransaction from detailed transaction logs', () => {
    ...
    //Transaction has an event that sugests it is and DCDTNFTCreate transaction
    transaction = new ShardTransaction();
    event.identifier = 'DCDTNFTCreate';
    event.topics = [
      "T0dTLTNmMTQwOA==",
      "Jvk=",
      "AQ==",
    ...
    //Transaction does not have any event that sugests it is and DCDTNFTCreate transaction
    transaction = new ShardTransaction();
  ...
  it('tryExtractNftIdentifierFromNftUpdateMetadataTransaction', () => {
    ...
    transaction = new ShardTransaction();
    transaction.data = 'RENEVE5GVFVwZGF0ZUF0dHJpYnV0ZXNANTQ0NTUzNTQ0MzRmNGMyZDYxNjI2MzY0QDAxQHRlc3RhdHRyaWJ1dGVz';
  ...
  it('trimOperations', () => {
    const operations: TransactionOperation[] = [
      new TransactionOperation({
        ...
        type: TransactionOperationType.dcdt,
