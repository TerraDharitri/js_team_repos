...
import { OperaWalletLoginButton } from '..';
import { checkIsLoggedInStore } from './helpers';

jest.mock('@terradharitri/sdk-opera-provider', () => {
  const { OperaProvider } = require('./mocks/mockOperaProvider');
  return {
    __esModule: true,
    OperaProvider
  };
...
const tokenLogin = {
  ...
  nativeAuthToken:
    'ZHJ0MWRtOXV4cGY1YXdrbjd1aGp1N3pqbjlsZGUwZGhhaHkwcWF4cXFsdTI2eGN1dXcyN3FxcnNhNHY2MzA.YUhSMGNEb3ZMMnh2WTJGc2FHOXpkQS5mZmY2N2QzMTQ3NmFkOTIwZDUzMDkzYTNhNGMyMTc4ZTE5ODE3OWIzNTY1NmVlZWZhNDE5MTA3ZmE3MThiNzgwLjg2NDAwLmV5SjBhVzFsYzNSaGJYQWlPakUyT1RBeE9EUXpNVE45.e4c98dd01020118b13db9dd5db9e5b56ff0c4a0141306918a9d3eea964a21ada5d566f58cdf6c921ed3405bf5685d1e87545dbcc86ea3c27a43aa3abee8c2b0e'
};
...
describe.skip('OperaLoginButton tests', () => {
  ...
  it('should perform simple login and redirect', async () => {
    const methods = renderWithProvider({
      children: <OperaWalletLoginButton callbackRoute={CALLBACK_ROUTE} />
    });

    const loginButton = await methods.findByTestId('operaLoginButton');
  ...
  it('should perform login with nativeAuth', async () => {
    const methods = renderWithProvider({
      children: (
        <OperaWalletLoginButton callbackRoute={CALLBACK_ROUTE} nativeAuth />
      )
    });
