...
import { DRTApiService } from '../../../services/dharitri-communication/drt.api.service';
...
@Injectable()
export class UserMetaDcdtComputeService {
    constructor(
    ...
    async farmTokenUSD(
        ...
        try {
            ...
            this.logger.error(
                ...
                e.stack,
                UserMetaDcdtComputeService.name,
            );
            return undefined;
        }
    }
    ...
    async lockedDcdtTokenUSD(nftToken: NftToken): Promise<UserLockedDcdtToken> {
        const decodedAttributes =
        ...
        let userDcdtToken: UserToken;
        const dcdtToken = new DcdtToken({
            identifier: decodedAttributes.originalTokenID,
            balance: nftToken.balance,
            decimals: nftToken.decimals,
        });
        if (pairAddress) {
            userDcdtToken = await this.userDcdtCompute.lpTokenUSD(
                dcdtToken,
                pairAddress,
            );
        } else {
            userDcdtToken = await this.userDcdtCompute.dcdtTokenUSD(dcdtToken);
        }

        return new UserLockedDcdtToken({
            ...nftToken,
            decodedAttributes,
            valueUSD: userDcdtToken.valueUSD,
        });
    }
    ...
    async lockedSimpleLpTokenUSD(
        ...
        const userDcdtToken = await this.userDcdtCompute.lpTokenUSD(
            new DcdtToken({
    ...
    async lockedTokenEnergyUSD(
        ...
        const userDcdtToken = await this.userDcdtCompute.dcdtTokenUSD(
            dcdtToken,
        );
