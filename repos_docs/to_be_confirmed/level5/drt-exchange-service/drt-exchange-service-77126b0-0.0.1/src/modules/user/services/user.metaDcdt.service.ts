...
@Injectable()
export class UserMetaDcdtService {
    constructor(
        private readonly userComputeService: UserMetaDcdtComputeService,
    ...
    async getUserLockedDcdtToken(
        userAddress: string,
        pagination: PaginationArgs,
        rawNfts?: NftToken[],
    ): Promise<UserLockedDcdtToken[]> {
        const lockedDcdtTokenIDs = await Promise.all(
        ...
        const nfts = rawNfts
            ? rawNfts.filter((nft) =>
                  lockedDcdtTokenIDs.includes(nft.collection),
              )
            : await this.contextGetter.getNftsForUser(
                  userAddress,
                  pagination.offset,
        ...
        return Promise.all(
            nfts.map((nft) => this.userComputeService.lockedDcdtTokenUSD(nft)),
        );
    }
        ...
        for (const userNft of userNFTs) {
            ...
            switch (userNftTokenType) {
                ...
                case NftTokenType.RedeemToken:
                    ...
                    break;
                case NftTokenType.LockedDcdtToken:
                    promises.push(
                        this.userComputeService.lockedDcdtTokenUSD(userNft),
                    );
                    break;
    ...
    private async getNftTokenTypeRaw(tokenID: string): Promise<NftTokenType> {
        ...
        for (const simpleLockAddress of scAddress.simpleLockAddress) {
            ...
            switch (tokenID) {
                case lockedTokenID:
                    return NftTokenType.LockedDcdtToken;
                case lpProxyTokenID:
