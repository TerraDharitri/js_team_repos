...
import { DRTDataApiService } from 'src/services/dharitri-communication/drt.data.api.service';
...
export class PairComputeService implements IPairComputeService {
    ...
    async computeTypeFromTokens(pairAddress: string): Promise<string> {
        ...
        const [firstTokenType, secondTokenType] = await Promise.all([
            this.tokenService.getDcdtTokenType(firstTokenID),
            this.tokenService.getDcdtTokenType(secondTokenID),
        ]);
