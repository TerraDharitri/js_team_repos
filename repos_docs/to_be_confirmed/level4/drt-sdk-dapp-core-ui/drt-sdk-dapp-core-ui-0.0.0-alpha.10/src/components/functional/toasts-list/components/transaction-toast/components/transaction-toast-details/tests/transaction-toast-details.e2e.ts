...
describe('transaction-toast-details', () => {
  it('renders correctly with transactions', async () => {
    ...
    const statusElement = await page.find('drt-transaction-toast-details >>> .transaction-details-status-text');
    expect(statusElement.textContent).toBe('3 Transactions');

    const transactionItems = await page.findAll('drt-transaction-toast-details >>> drt-transaction-toast-details-body');
  ...
  it('expands when status is clicked', async () => {
    ...
    let detailsList = await page.find('drt-transaction-toast-details >>> .transaction-details-list');
    expect(detailsList.getAttribute('class')).not.toContain('expanded');

    const statusElement = await page.find('drt-transaction-toast-details >>> .transaction-details-status');
    await statusElement.click();

    await page.waitForChanges();

    detailsList = await page.find('drt-transaction-toast-details >>> .transaction-details-list');
  ...
  it('shows "Show more" button when there are more transactions than maxShownTransactions', async () => {
    ...
    const showMoreButton = await page.find('drt-transaction-toast-details >>> .show-more-button');
    ...
    const showMoreButtonAfter = await page.find('drt-transaction-toast-details >>> .show-more-button');
