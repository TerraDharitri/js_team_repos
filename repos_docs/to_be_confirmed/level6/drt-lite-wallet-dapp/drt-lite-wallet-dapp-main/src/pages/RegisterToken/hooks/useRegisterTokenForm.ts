...
import { networks } from 'config/config.devnet';
import { capitalize, addressIsDrt } from 'helpers';
...
export const useRegisterTokenForm = () => {
  ...
  const [sendType, setSendType] = useState(SendTypeEnum.dcdt);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      [RegisterTokenFormFieldsEnum.type]: SendTypeEnum.dcdt
    },
