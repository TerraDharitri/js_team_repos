...
import { ElasticService, ElasticQuery, QueryOperator, QueryType, QueryConditionOptions, ElasticSortOrder, ElasticSortProperty, TermsQuery, RangeGreaterThanOrEqual, MatchQuery } from "@terradharitri/sdk-nestjs-elastic";
...
export class ElasticIndexerService implements IndexerInterface {
  private nonFungibleDcdtTypes: NftType[] = [NftType.NonFungibleDCDT, NftType.NonFungibleDCDTv2, NftType.DynamicNonFungibleDCDT];
  private semiFungibleDcdtTypes: NftType[] = [NftType.SemiFungibleDCDT, NftType.DynamicSemiFungibleDCDT];
  private metaDcdtTypes: NftType[] = [NftType.MetaDCDT, NftType.DynamicMetaDCDT];
  ...
  async getAccountDcdtHistory(address: string, pagination: QueryPagination, filter: AccountHistoryFilter): Promise<any[]> {
    const elasticQuery: ElasticQuery = this.indexerHelper.buildAccountHistoryFilterQuery(address, undefined, filter)
      .withPagination(pagination)
      .withSort([{ name: 'timestamp', order: ElasticSortOrder.descending }]);
  ...
  async getAccountDcdtHistoryCount(address: string, filter?: AccountHistoryFilter): Promise<number> {
    const elasticQuery: ElasticQuery = this.indexerHelper.buildAccountHistoryFilterQuery(address, undefined, filter);
  ...
  private buildTokenFilter(query: ElasticQuery, filter: TokenFilter): ElasticQuery {
    if (filter.includeMetaDCDT === true) {
      query = query.withMustMultiShouldCondition([TokenType.FungibleDCDT, TokenType.MetaDCDT], type => QueryType.Match('type', type));
    } else {
  ...
  async getAccountsDcdtByCollection(identifiers: string[], pagination?: QueryPagination) {
    if (identifiers.length === 0) {
      return [];
    }
  ...
  async getNfts(pagination: QueryPagination, filter: NftFilter, identifier?: string): Promise<any[]> {
    ...
    if (elasticNfts.length === 0 && identifier !== undefined) {
      elasticNfts = await this.elasticService.getList('accountsdcdt', 'identifier', ElasticQuery.create().withMustMatchCondition('identifier', identifier, QueryOperator.AND));
    }
    return elasticNfts;
  }
  ...
  async getAllTokensMetadata(action: (items: any[]) => Promise<void>): Promise<void> {
    const query = ElasticQuery.create()
      ...
      .withMustMultiShouldCondition([DcdtType.NonFungibleDCDT, DcdtType.SemiFungibleDCDT], type => QueryType.Match('type', type))
      .withPagination({ from: 0, size: 10000 });

    return await this.elasticService.getScrollableList('tokens', 'identifier', query, action);
  }

  async getDcdtAccountsCount(identifier: string): Promise<number> {
    const elasticQuery: ElasticQuery = ElasticQuery.create()
      .withCondition(QueryConditionOptions.must, [QueryType.Match("token", identifier, QueryOperator.AND)]);
  ...
  async getDcdtProperties(identifier: string): Promise<any> {
    return await this.elasticService.getItem(
      'tokens',
      'identifier',
      identifier
  ...
  async getAllFungibleTokens(): Promise<any[]> {
    const query = ElasticQuery.create()
      .withMustMatchCondition('type', TokenType.FungibleDCDT)
