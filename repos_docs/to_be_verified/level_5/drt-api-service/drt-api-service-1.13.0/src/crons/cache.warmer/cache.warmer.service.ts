...
export class CacheWarmerService {
  ...
  async handleDcdtTokenInvalidations() {
    ...
    await this.invalidateKey(CacheInfo.AllDcdtTokens.key, tokens, CacheInfo.AllDcdtTokens.ttl);
  }
  ...
  async handleTokenAssetsExtraInfoInvalidations() {
    ...
    for (const identifier of Object.keys(assets)) {
      ...
      if (asset.extraTokens || token.type === TokenType.MetaDCDT) {
        const accounts = await this.dcdtService.countAllDistinctAccounts([identifier, ...(asset.extraTokens ?? [])]);
