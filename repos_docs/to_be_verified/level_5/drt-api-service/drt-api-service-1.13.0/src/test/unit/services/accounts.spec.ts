...
import { AccountDcdtHistory } from "src/endpoints/accounts/entities/account.dcdt.history";
...
describe('Account Service', () => {
      ...
      providers: [
        AccountService,
        {
          ...
          useValue: {
            ...
            getAccountDcdtHistory: jest.fn(),
            getAccountDcdtHistoryCount: jest.fn(),
          },
        },
        {
  ...
  describe('getAccountTokenHistory', () => {
    ...
    it('should return the account history for a specific token', async () => {
      ...
      const expectedResult = elasticResult.map(item => ApiUtils.mergeObjects(new AccountDcdtHistory(), item));
      expect(result).toEqual(expectedResult);
    });
  });
  ...
  describe('getAccountDcdtHistory', () => {
    ...
    it('should return the account tokens history', async () => {
      jest.spyOn(indexerService, 'getAccountDcdtHistory').mockResolvedValue(elasticResult);

      const result = await service.getAccountDcdtHistory(address, pagination, filter);

      expect(indexerService.getAccountDcdtHistory).toHaveBeenCalledWith(address, pagination, filter);
    ...
    it('should return an empty array if no token history is found', async () => {
      jest.spyOn(indexerService, 'getAccountDcdtHistory').mockResolvedValue([]);
  ...
  describe('getAccountDcdtHistoryCount', () => {
    ...
    it('should return the account dcdt history count', async () => {
      jest.spyOn(indexerService, 'getAccountDcdtHistoryCount').mockResolvedValue(dcdtHistoryCount);

      const result = await service.getAccountDcdtHistoryCount(address, filter);

      expect(indexerService.getAccountDcdtHistoryCount).toHaveBeenCalledWith(address, filter);
      expect(result).toEqual(dcdtHistoryCount);
    });
  });
