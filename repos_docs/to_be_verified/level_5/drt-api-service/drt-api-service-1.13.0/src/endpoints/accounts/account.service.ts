...
export class AccountService {
  ...
  async getAccountTokenHistory(address: string, tokenIdentifier: string, pagination: QueryPagination, filter: AccountHistoryFilter): Promise<AccountDcdtHistory[]> {
    const elasticResult = await this.indexerService.getAccountTokenHistory(address, tokenIdentifier, pagination, filter);
    return elasticResult.map(item => ApiUtils.mergeObjects(new AccountDcdtHistory(), item));
  }

  async getAccountDcdtHistory(address: string, pagination: QueryPagination, filter: AccountHistoryFilter): Promise<AccountDcdtHistory[]> {
