...
import { MultiDcdtTransactionType, TransactionTypesEnum } from 'types';
...
export function parseMultiDcdtTransferData(data?: string) {
  ...
  try {
    if (
      data?.startsWith(TransactionTypesEnum.MultiDCDTNFTTransfer) &&
      data?.includes('@')
    ) {
            ...
            switch (index) {
              ...
              case 1: {
                ...
                if (encodedNonce && encodedNonce !== '00') {
                  ...
                  transaction.type = TransactionTypesEnum.dcdtTransaction;
                }
