...
describe('getTransactionMethod', () => {
  ...
  it(`returns default value "Transaction" when the transaction is a "${TransactionActionsEnum.transfer}" and the action's category is "${TransactionActionCategoryEnum.dcdtNft}"`, () => {
    const transaction: ServerTransactionType = {
      ...baseTransactionMock,
      action: {
        category: TransactionActionCategoryEnum.dcdtNft,
  ...
  it(`returns the transaction method read from the action field when the transaction is not a "${TransactionActionsEnum.transfer}" or the action's category is not "${TransactionActionCategoryEnum.dcdtNft}"`, () => {
    const transaction: ServerTransactionType = {
      ...baseTransactionMock,
      action: {
