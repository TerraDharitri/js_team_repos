...
describe('SnapshotsService', () => {
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            ...
            addLockedRewaForAddress: jest.fn(),
          },
        },
        {
          provide: ApiService,
  ...
  describe('addRecordsToIndexer', () => {
    it('should call getCurrentEpoch and addLockedRewaForAddress for each user', async () => {
      const users = {
        'drt1abc': [{ providerName: 'provider1', lockedRewa: new BigNumber(1000) }],
        'drt1dfg': [{ providerName: 'provider2', lockedRewa: new BigNumber(2000) }],
      };
      ...
      expect(elasticIndexer.addLockedRewaForAddress).toHaveBeenCalledWith('drt1abc', currentEpoch, users['drt1abc']);
      expect(elasticIndexer.addLockedRewaForAddress).toHaveBeenCalledWith('drt1dfg', currentEpoch, users['drt1dfg']);
      expect(elasticIndexer.addLockedRewaForAddress).toHaveBeenCalledTimes(2);
    });
    ...
    it('should handle no users without any errors', async () => {
      ...
      expect(provider.getCurrentEpoch).toHaveBeenCalled();
      expect(elasticIndexer.addLockedRewaForAddress).not.toHaveBeenCalled();
    });
  });
});
