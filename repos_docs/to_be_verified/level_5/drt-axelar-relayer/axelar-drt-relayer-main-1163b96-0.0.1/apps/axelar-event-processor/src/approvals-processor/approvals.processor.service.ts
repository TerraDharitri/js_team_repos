...
export class ApprovalsProcessorService {
  ...
  async handleNewTasksRaw() {
    ...
    this.logger.debug(`Trying to process tasks for dharitri starting from id: ${lastTaskUUID}`);
  ...
  private async processGatewayTxTask(externalData: string, retry: number = 0) {
    // The Amplifier for  encodes the executeData as hex, we need to decode it to string
  ...
  private async processRefundTask(response: RefundTask) {
    ...
    try {
      ...
      if (tokenBalance.lt(response.remainingGasBalance.amount)) {
        throw new Error(
          `Insufficient balance for token ${response.remainingGasBalance.tokenID || CONSTANTS.REWA_IDENTIFIER}` +
    ...
    const transaction = this.gasServiceContract.refund(
      ...
      response.remainingGasBalance.tokenID || CONSTANTS.REWA_IDENTIFIER,
