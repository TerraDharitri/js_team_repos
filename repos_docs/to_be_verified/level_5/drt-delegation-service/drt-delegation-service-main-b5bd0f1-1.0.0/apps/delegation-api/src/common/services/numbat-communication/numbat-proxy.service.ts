import { ContractFunction, Address, AddressValue } from '@terradharitri/sdk-core';
import { numbatConfig } from '../../../config';
import { Inject, Injectable } from '@nestjs/common';
import { Query } from '@terradharitri/sdk-core';
import { WINSTON_MODULE_PROVIDER } from 'nest-winston';
import { Logger } from 'winston';
import { CacheManagerService } from '../cache-manager/cache-manager.service';
import BigNumber from 'bignumber.js';
import { ContractQueryResponse, NetworkStatus, ProxyNetworkProvider, NetworkConfig } from '@terradharitri/sdk-network-providers';

@Injectable()
export class NumbatProxyService {
  private readonly proxy: ProxyNetworkProvider;
  constructor(
    private cacheManager: CacheManagerService,
    @Inject(WINSTON_MODULE_PROVIDER) private readonly logger: Logger
  ) {
    this.proxy = new ProxyNetworkProvider(numbatConfig.gateway, {
      timeout: 60000,
    });
  }
  ...
  async getAllContractAddresses(): Promise<ContractQueryResponse> {
    ...
    const query =
      new Query({
        address: new Address(numbatConfig.stakingContract),
    ...
    this.logger.info('getContractList', {
      path: 'numbat-proxy.service.getContractList',
  ...
  async getContractConfig(delegationContract: string): Promise<ContractQueryResponse> {
    ...
    this.logger.info('getContractConfig', {
      path: 'numbat-proxy.service.getContractConfig',
      delegationContract,
      returnCode: result.returnCode,
      returnMessage: result.returnMessage,
    });
  ...
  async getBlsKeys(delegationContract: string): Promise<ContractQueryResponse> {
    const { auctionContract } = numbatConfig;
    ...
    this.logger.info('getBlsKeysStatus', {
      path: 'numbat-proxy.service.getBlsKeys',
      delegationContract,
      returnCode: result.returnCode,
      returnMessage: result.returnMessage,
    });
  ...
  private async getUserContractData(method: string, address: string, delegationContract: string) {
    ...
    this.logger.info(method, {
      path: 'numbat-proxy.service.getUserDelegationMethod',
      method,
      delegationContract,
      address,
      returnCode: result.returnCode,
  ...
  async getGlobalDelegationMethod(method: string, delegationContract: string): Promise<ContractQueryResponse> {
    ...
    const result = await this.proxy.queryContract(query);
    this.logger.info(method, {
      path: 'numbat-proxy.service.getGlobalDelegationMethod',
      method,
      delegationContract,
      returnCode: result.returnCode,
      returnMessage: result.returnMessage,
  ...
  async getContractMetaData(delegationContract: string): Promise<ContractQueryResponse> {
    ...
    this.logger.info('getMetaData', {
      path: 'numbat-proxy.service.getContractMetaData',
      delegationContract,
      returnCode: result.returnCode,
      returnMessage: result.returnMessage,
    });
  ...
  async getUserUnDelegatedList(address: string, delegationContract: string): Promise<ContractQueryResponse> {
    ...
    this.logger.info('getUserUnDelegatedList', {
      path: 'numbat-proxy.service.getUserUnDelegatedList',
      delegationContract,
      address,
      returnCode: result.returnCode,
      returnMessage: result.returnMessage,
  ...
  async getQueueRegisterNonceAndRewardAddress(): Promise<ContractQueryResponse> {
    const { auctionContract, blsRewardsAddress } = numbatConfig;
