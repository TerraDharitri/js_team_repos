...
import { NetworkStatus } from '@terradharitri/sdk-network-providers';

@Injectable()
export class DelegationService {
  constructor(
    private numbatElasticService: NumbatElasticService,
  ...
  private async invalidateUserInfoForContract(address: string, contract: string) {
    const networkStatus: NetworkStatus = await this.numbatProxyService.getNetworkStatus();
  ...
  async getAllContractDataForUser(address: string, forceRefresh?: boolean): Promise<Delegation[]> {
    ...
    if (!allActiveContracts) {
      allActiveContracts = await this.numbatElasticService.getAddressActiveContracts(address);
    }
  ...
  async getUserActiveStake(contract: string, address: string): Promise<string> {
    try {
      return QueryResponseHelper.handleQueryAmountResponse(
        await this.numbatProxyService.getUserActiveStake(
          contract,
          address
        ));
    } catch (e) {
  ...
  private async getUserUnBondable(address: string, contract: string) {
    try {
      return QueryResponseHelper.handleQueryAmountResponse(
        await this.numbatProxyService.getUserUnBondable(address, contract)
      );
    } catch (e) {
  ...
  private async getClaimableRewards(address: string, contract: string) {
    try {
      return QueryResponseHelper.handleQueryAmountResponse(
        await this.numbatProxyService.getClaimableRewards(address, contract)
      );
    } catch (e) {
  ...
  private async getUserUnDelegatedList(contract: string, address: string): Promise<UserUndelegatedListDto> {
    try {
      const scResponse = await this.numbatProxyService.getUserUnDelegatedList(
        address,
        contract
      );
      ...
      const networkConfig = await this.numbatProxyService.getNetworkConfig();
      const networkStatus = await this.numbatProxyService.getNetworkStatus();
  ...
  async getDelegationForUser(contract: string, address: string): Promise<Delegation | null> {
    const delegation = await this.numbatElasticService.getDelegationForAddressAndContract(address, contract);
