...
import { BaseDcdtToken } from 'src/modules/tokens/models/dcdtToken.model';
import {
...
export class AutoRouterService {
    ...
    async singleSwap(
        ...
        tokenOutID: string,
        pair: PairModel,
        tokenInMetadata: BaseDcdtToken,
        tokenOutMetadata: BaseDcdtToken,
        swapType: SWAP_TYPE,
    ...
    private async getAllActivePairs(): Promise<PairModel[]> {
        ...
        return activePairs.map((pair, index) => {
            return new PairModel({
                ...
                firstToken: BaseDcdtToken.toDcdtToken(
                    tokenMap.get(pair.firstTokenID),
                ),
                secondToken: BaseDcdtToken.toDcdtToken(
