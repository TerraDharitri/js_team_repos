...
export class PairRewardTokensResolver extends GenericResolver {
    ...
    @ResolveField()
    async poolRewards(parent: PairRewardTokensModel): Promise<DcdtToken[]> {
        return Promise.all([
    ...
    @ResolveField()
    async dualFarmReward(parent: PairRewardTokensModel): Promise<DcdtToken> {
...
export class PairResolver {
    ...
    @ResolveField()
    async firstToken(parent: PairModel): Promise<DcdtToken> {
        return this.pairAbiLoader.firstTokenLoader.load(parent.address);
    }

    @ResolveField()
    async secondToken(parent: PairModel): Promise<DcdtToken> {
        return this.pairAbiLoader.secondTokenLoader.load(parent.address);
    }

    @ResolveField()
    async liquidityPoolToken(parent: PairModel): Promise<DcdtToken> {
    ...
    @Query(() => DcdtTokenPayment)
    async updateAndGetSafePrice(
        @Args('pairAddress') pairAddress: string,
        @Args('dcdtTokenPayment') dcdtTokenPayment: DcdtTokenPayment,
