...
export class PairService {
    ...
    async getFirstToken(pairAddress: string): Promise<DcdtToken> {
        const firstTokenID = await this.pairAbi.firstTokenID(pairAddress);
        return this.tokenService.tokenMetadata(firstTokenID);
    }

    async getAllFirstTokens(pairAddresses: string[]): Promise<DcdtToken[]> {
    ...
    async getSecondToken(pairAddress: string): Promise<DcdtToken> {
        const secondTokenID = await this.pairAbi.secondTokenID(pairAddress);
        return this.tokenService.tokenMetadata(secondTokenID);
    }

    async getAllSecondTokens(pairAddresses: string[]): Promise<DcdtToken[]> {
        const tokenIDs = await getAllKeys<string>(
            this.cachingService,
            pairAddresses,
            'pair.secondTokenID',
    ...
    async getLpToken(pairAddress: string): Promise<DcdtToken> {
    ...
    async getAllLpTokens(pairAddresses: string[]): Promise<DcdtToken[]> {
    ...
    async isPairDcdtToken(tokenID: string): Promise<boolean> {
        const pairsAddress = await this.routerAbi.pairsAddress();
