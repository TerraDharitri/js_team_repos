...
import {
    DcdtTokenPayment,
    DcdtTokenPaymentStruct,
    DcdtTokenType,
} from 'src/models/dcdtTokenPayment.model';
import {
    Address,
    BigUIntValue,
    EnumValue,
    ...
    implements IPairAbiService
{
    ...
    async getFirstTokenIDRaw(pairAddress: string): Promise<string> {
        const contract = await this.drtProxy.getPairSmartContract(pairAddress);
    ...
    async getLpTokenIDRaw(pairAddress: string): Promise<string> {
        ...
        return lpTokenID === drtConfig.rewaIdentifier || lpTokenID === ''
            ? undefined
            : lpTokenID;
    }
    ...
    async getInitialLiquidityAdderRaw(pairAddress: string): Promise<string> {
        ...
        try {
            ...
            const queryResponse = await this.drtProxy
                .getService()
                .queryContract(query);
            if (
    ...
    async getRouterAddressRaw(address: string): Promise<string> {
        const contract = await this.drtProxy.getPairSmartContract(address);
        const interaction: Interaction =
    ...
    async safePrice(
        pairAddress: string,
        dcdtTokenPayment: DcdtTokenPayment,
    ): Promise<DcdtTokenPayment> {
        return this.updateAndGetSafePrice(pairAddress, dcdtTokenPayment);
    }
    ...
    async updateAndGetSafePrice(
        ...
        const interaction: Interaction = contract.methods.updateAndGetSafePrice(
            [
                new Struct(DcdtTokenPaymentStruct.getStructure(), [
                    new Field(
                        new EnumValue(
                            DcdtTokenType.getEnum(),
                            DcdtTokenType.getEnum().getVariantByDiscriminant(
                                dcdtTokenPayment.tokenType,
                            ),
                            [],
                        ),
                        'token_type',
                    ...
                    new Field(
                        new TokenIdentifierValue(
                            Buffer.from(dcdtTokenPayment.tokenID).toString(),
                        ),
                        'token_identifier',
                    ),
                    new Field(
                        new U64Value(new BigNumber(dcdtTokenPayment.nonce)),
                        'token_nonce',
                    ),
                    new Field(
                        new BigUIntValue(
                            new BigNumber(dcdtTokenPayment.amount),
                        ),
                        'amount',
                    ),
                ]),
        ...
        return new DcdtTokenPayment({
            tokenType: DcdtTokenType.getEnum().getVariantByName(
