...
export class TokenService {
    ...
    async getTokens(filters: TokensFiltersArgs): Promise<DcdtToken[]> {
        ...
        if (filters.type) {
            const tokenTypes = await this.getAllDcdtTokensType(tokenIDs);
    ...
    ): Promise<CollectionType<DcdtToken>> {
        let tokenIDs = await this.getUniqueTokenIDs(filters.enabledSwaps);
    ...
    async getDcdtTokenType(tokenID: string): Promise<string> {
        ...
        if (pairAddress) {
            return DcdtTokenType.FungibleLpToken;
        }

        return this.tokenRepository.getTokenType(tokenID);
    }

    async getAllDcdtTokensType(tokenIDs: string[]): Promise<string[]> {
        return getAllKeys<string>(
            this.cachingService,
            tokenIDs,
            'token.getDcdtTokenType',
            this.getDcdtTokenType.bind(this),
            CacheTtlInfo.Token,
        );
    }
    ...
    async tokenMetadataRaw(tokenID: string): Promise<DcdtToken> {
    ...
    async getAllBaseTokensMetadata(
        ...
        return getAllKeys<BaseDcdtToken>(
            this.cachingService,
            tokenIDs,
    ...
    async baseTokenMetadataRaw(tokenID: string): Promise<BaseDcdtToken> {
