...
export class EventsProcessorService {
    ...
    private async getSwapAndBurnEvents(
        ...
        await this.elasticEventsService.getEvents(
            [
                ...
                TRANSACTION_EVENTS.DCDT_LOCAL_BURN,
            ],
            lastProcessedTimestamp,
            currentTimestamp,
            processEventsAction,
    ...
    private async getExitFarmAndBurnEvents(
        ...
        await this.elasticEventsService.getEvents(
            ['exitFarm', TRANSACTION_EVENTS.DCDT_LOCAL_BURN],
            lastProcessedTimestamp,
            currentTimestamp,
            processEventsAction,
            500,
    ...
    private processSwapTransactionsEvents(
        ...
        for (const txHash of txHashes) {
            ...
            const burnEvents = events
                .filter(
                    (event) =>
                        event.identifier === TRANSACTION_EVENTS.DCDT_LOCAL_BURN,
                )
                .map((event) => new DcdtLocalBurnEvent(event));
    ...
    private processExitFarmTransactionsEvents(
        ...
        for (const txHash of txHashes) {
            ...
            const burnEvents: DcdtLocalBurnEvent[] = [];
            ...
            events.forEach((event) => {
                switch (event.identifier) {
                    case 'exitFarm':
                        ...
                        break;
                    case TRANSACTION_EVENTS.DCDT_LOCAL_BURN:
                        burnEvents.push(new DcdtLocalBurnEvent(event));
                        break;
                    default:
                        break;
                }
    ...
    private getBurnedFee(dcdtLocalBurnEvents: DcdtLocalBurnEvent[]): string {
        let fee = new BigNumber(0);
        for (const localBurn of dcdtLocalBurnEvents) {
    ...
    private getBurnedPenalty(
        exitFarmEvent: BaseFarmEvent | ExitFarmEventV2,
        dcdtLocalBurnEvents: DcdtLocalBurnEvent[],
    ): string {
