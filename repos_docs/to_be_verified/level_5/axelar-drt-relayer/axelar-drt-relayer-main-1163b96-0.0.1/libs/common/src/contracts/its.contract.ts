...
const DEFAULT_DCDT_ISSUE_COST = '50000000000000000'; // 0.05 REWA
...
export class ItsContract {
  ...
  execute(
    ...
    // The second time this transaction is executed it needs to contain and REWA transfer for issuing DCDT
    if (messageType === MESSAGE_TYPE_DEPLOY_INTERCHAIN_TOKEN && executedTimes === 1) {
      interaction.withValue(TokenTransfer.rewaFromBigInteger(DEFAULT_DCDT_ISSUE_COST));
    }
