...
export class NftMarketplaceAbiService {
  ...
  async createAuction(ownerAddress: string, args: CreateAuctionRequest): Promise<TransactionNode> {
    ...
    return contract.methodsExplicit
      ...
      .withSingleDCDTNFTTransfer(TokenTransfer.metaDcdtFromBigInteger(collection, parseInt(nonce, 16), new BigNumber(args.quantity)))
  ...
  async bid(ownerAddress: string, request: BidRequest): Promise<TransactionNode> {
    ...
    return request.paymentTokenIdentifier !== drtConfig.rewa
      ? await this.bidWithDcdt(ownerAddress, request, contract, auction.marketplaceAuctionId)
      : await this.bidWithRewa(ownerAddress, request, contract, auction.marketplaceAuctionId);
  }
  ...
  async createOffer(ownerAddress: string, request: CreateOfferRequest): Promise<TransactionNode> {
    ...
    if (request.paymentToken !== drtConfig.rewa) {
      return intermediateInteraction
        .withSingleDCDTTransfer(TokenTransfer.fungibleFromBigInteger(request.paymentToken, new BigNumber(request.paymentAmount)))
        .withSender(Address.fromString(ownerAddress))
        .buildTransaction()
        .toPlainObject();
    }
    return intermediateInteraction
      .withValue(TokenTransfer.rewaFromBigInteger(request.paymentAmount))
      .withSender(Address.fromString(ownerAddress))
      .buildTransaction()
      .toPlainObject();
  }
  ...
  private async acceptSingleOffer(ownerAddress: string, offerId: number): Promise<TransactionNode> {
    ...
    if (asset.type === NftTypeEnum.SemiFungibleDCDT && parseInt(asset.balance) < parseInt(offer.boughtTokensNo)) {
    ...
    return contract.methodsExplicit
      ...
      .withSingleDCDTNFTTransfer(
        TokenTransfer.metaDcdtFromBigInteger(
          collection,
          parseInt(nonce, 16),
          asset.type === NftTypeEnum.SemiFungibleDCDT ? new BigNumber(offer.boughtTokensNo) : new BigNumber(1),
        ),
      )
      .withChainID(drtConfig.chainID)
      .withGasLimit(gas.withdraw)
  ...
  async buySft(ownerAddress: string, request: BuySftRequest): Promise<TransactionNode> {
    ...
    return request.paymentTokenIdentifier !== drtConfig.rewa
      ? await this.buySftWithDcdt(ownerAddress, request, contract, auction.marketplaceAuctionId)
      : await this.buySftWithRewa(ownerAddress, request, contract, auction.marketplaceAuctionId);
  }
  ...
  private async bidWithRewa(
    ...
    return contract.methodsExplicit
      ...
      .withValue(TokenTransfer.rewaFromBigInteger(request.price))
      .withChainID(drtConfig.chainID)
      .withGasLimit(gas.bid)
      .withSender(Address.fromString(ownerAddress))
      .buildTransaction()
  ...
  private async bidWithDcdt(
    ...
    return contract.methodsExplicit
      .bid([new U64Value(new BigNumber(marketplaceAuctionId)), BytesValue.fromUTF8(collection), BytesValue.fromHex(nonce)])
      .withSingleDCDTTransfer(TokenTransfer.fungibleFromBigInteger(request.paymentTokenIdentifier, new BigNumber(request.price)))
      .withChainID(drtConfig.chainID)
      .withGasLimit(gas.bid)
      .withSender(Address.fromString(ownerAddress))
      .buildTransaction()
