...
import {
  AuctionTypeEnum,
  NumbatSwapAuctionTypeEnum,
} from 'src/modules/auctions/models';
import { AuctionTokenEvent } from 'src/modules/rabbitmq/entities/auction';
import { NumbatSwapAuctionEvent } from 'src/modules/rabbitmq/entities/auction/numbatnftswap/numbatswap-auction.event';
...
export class AuctionStartedSummary extends ReindexGenericSummary {
  ...
  private static getTopics(event: MarketplaceEventsEntity): any {
    ...
    if (event.hasEventIdentifier(NumbatNftsSwapAuctionEventEnum.NftSwap)) {
      try {
        const topics = new NumbatSwapAuctionEvent(genericEvent).getTopics();
