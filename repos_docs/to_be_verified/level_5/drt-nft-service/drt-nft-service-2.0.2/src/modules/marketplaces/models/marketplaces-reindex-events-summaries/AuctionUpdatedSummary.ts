...
import { NumbatSwapUpdateEvent } from 'src/modules/rabbitmq/entities/auction/numbatnftswap/numbatswap-updateAuction.event';
  ...
  private static getTopics(event: MarketplaceEventsEntity): any {
    ...
    if (
      event.hasOneOfEventIdentifiers([
        NumbatNftsSwapAuctionEventEnum.NftSwapUpdate,
        NumbatNftsSwapAuctionEventEnum.NftSwapExtend,
      ])
    ) {
      return new NumbatSwapUpdateEvent(genericEvent).getTopics();
    }
