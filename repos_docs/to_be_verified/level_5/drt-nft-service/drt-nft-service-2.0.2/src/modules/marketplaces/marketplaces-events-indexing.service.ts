...
import { BinaryUtils, ElasticQuery, Locker } from '@terradharitri/sdk-nestjs';
import {
...
export class MarketplaceEventsIndexingService {
  ...
  private async addGenericElasticDataToDbAndReturnNewestTimestamp(
    ...
    await this.drtElasticService.getScrollableList(
      elasticCollection,
      'identifier',
      elasticQuery,
      async (items) => {
  ...
  private isUnknownMarketplaceEvent(eventIdentifier: any): boolean {
    return (
      ...
      !Object.values(NumbatNftsSwapAuctionEventEnum).includes(eventIdentifier)
    );
  }
}
