...
import { NumbatSwapWithdrawEvent } from 'src/modules/rabbitmq/entities/auction/numbatnftswap/numbatswap-withdraw.event';
...
export class ReindexAuctionClosedSummary extends ReindexGenericSummary {
  ...
  private static getTopics(event: MarketplaceEventsEntity): any {
    ...
    if (event.hasEventIdentifier(NumbatNftsSwapAuctionEventEnum.WithdrawSwap)) {
      return new NumbatSwapWithdrawEvent(genericEvent).getTopics();
    }
