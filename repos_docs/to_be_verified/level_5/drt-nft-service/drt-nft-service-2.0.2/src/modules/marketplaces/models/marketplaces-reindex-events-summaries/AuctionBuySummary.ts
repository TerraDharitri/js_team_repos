...
import { NumbatSwapBuyEvent } from 'src/modules/rabbitmq/entities/auction/numbatnftswap/numbatswap-buy.event';
...
export class AuctionBuySummary extends ReindexGenericSummary {
  ...
  static fromBuySftEventAndTx(
    ...
    if (
      event.hasOneOfEventTopicIdentifiers([
        ...
        NumbatNftsSwapAuctionEventEnum.UpdateListing,
      ])
    ) {
      return;
    }
  ...
  private static getTopics(
    ...
    try {
      ...
      if (event.hasEventIdentifier(NumbatNftsSwapAuctionEventEnum.Purchase)) {
        return new NumbatSwapBuyEvent(genericEvent).getTopics();
      }
