...
export class NftScamElasticService {
  constructor(
    private drtService: DrtElasticService,
  ...
  async getNftWithScamInfoFromElastic(identifier: string): Promise<any> {
    ...
    try {
      ...
      await this.drtService.getScrollableList(
        'tokens',
        'identifier',
        query,
        async (items) => {
  ...
  async setBulkNftScamInfoInElastic(
    ...
    if (nfts.length > 0) {
      try {
        await this.drtService.bulkRequest(
          'tokens',
  ...
  async updateBulkNftScamInfoInElastic(updates: string[]): Promise<void> {
    if (updates.length > 0) {
      try {
        await this.drtService.bulkRequest('tokens', updates, '?timeout=1m');
      } catch (error) {
        this.logger.error('Error when bulk updating nft scam info in Elastic', {
  ...
  async setNftScamInfoManuallyInElastic(
    ...
    try {
      ...
      await this.drtService.bulkRequest('tokens', updates);
    } catch (error) {
      this.logger.error(
