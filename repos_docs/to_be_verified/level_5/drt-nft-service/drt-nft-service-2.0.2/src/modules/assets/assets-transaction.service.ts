...
export class AssetsTransactionService {
  ...
  async burnQuantity(
    ...
    const [nft, drtStats] = await Promise.all([
      this.drtApiService.getNftByIdentifier(request.identifier),
      this.getOrSetAproximateDrtStats(),
    ]);
  ...
  async transferNft(
    ...
    response.gasLimit = Math.max(
      drtConfig.transferMinCost + response.data.length * drtConfig.pricePerByte,
      gas.nftTransfer,
    );
    return {
      ...response,
