...
import { AssetsHistoryNumbatNftsSwapEventsService } from './services/assets-history.nfts-swap-auction.service';
...
export class AssetsHistoryService {
  constructor(
    ...
    private readonly assetsHistoryNumbatNftsSwapEventsService: AssetsHistoryNumbatNftsSwapEventsService,
  ...
  private mapLogs(
    ...
    switch (eventCategory) {
      ...
      case NftEventTypeEnum.NumbatNftsSwapAuctionEventEnum: {
        this.addHistoryLog(
          historyLog,
          this.assetsHistoryNumbatNftsSwapEventsService.mapNumbatNftsSwapEventLog(
            eventType,
            mainEvent,
          ),
        );
  ...
  private getEventType(res: any, index: number): [string, string, any] {
    ...
    for (let i = 0; i < relatedEvents?.length; i++) {
      ...
      for (let j = 0; j < events.length; j++) {
        ...
        if (
          eventIdentifier !== NftEventEnum.DCDTNFTTransfer &&
          eventIdentifier !== NftEventEnum.MultiDCDTNFTTransfer &&
        ...
        if (
          Object.values(NumbatNftsSwapAuctionEventEnum).includes(
            eventIdentifier,
          )
        ) {
          return [
            NftEventTypeEnum.NumbatNftsSwapAuctionEventEnum,
            eventIdentifier,
            relatedEvents[i],
          ];
        }
