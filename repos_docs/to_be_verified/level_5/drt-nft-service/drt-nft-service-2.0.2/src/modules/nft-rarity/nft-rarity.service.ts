...
import { Locker } from '@terradharitri/sdk-nestjs';
@Injectable()
  ...
  private async getAllCollectionNftsWithCustomRanksFromAPI(
    ...
    try {
      ...
      const preferredAlgorithm =
        await this.drtApiService.getCollectionPreferredAlgorithm(
          collectionTicker,
        );
      if (preferredAlgorithm === 'custom') {
        const customRanks = await this.drtApiService.getCollectionCustomRanks(
          collectionTicker,
        );
        return [
