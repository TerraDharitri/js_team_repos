...
import { NumbatSwapBuyEvent } from '../rabbitmq/entities/auction/numbatnftswap/numbatswap-buy.event';
...
export class BuyEventParser {
  ...
  private getEventAndTopics(event: any, hash: string) {
    if (event.identifier === NumbatNftsSwapAuctionEventEnum.Purchase) {
      if (
      ...
      const buySftEvent = new NumbatSwapBuyEvent(event);
