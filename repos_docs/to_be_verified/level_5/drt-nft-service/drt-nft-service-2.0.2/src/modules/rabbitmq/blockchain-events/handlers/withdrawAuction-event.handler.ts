...
import { NumbatSwapWithdrawEvent } from '../../entities/auction/numbatnftswap/numbatswap-withdraw.event';
...
export class WithdrawAuctionEventHandler {
  ...
  private getEventAndTopics(event: any) {
    if (event.identifier === NumbatNftsSwapAuctionEventEnum.WithdrawSwap) {
      const withdraw = new NumbatSwapWithdrawEvent(event);
