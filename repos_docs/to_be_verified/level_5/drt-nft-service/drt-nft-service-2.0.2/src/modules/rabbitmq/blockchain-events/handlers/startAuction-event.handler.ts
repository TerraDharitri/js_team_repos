...
import { NUMBATNFTSWAP_KEY, ENEFTOR_KEY } from 'src/utils/constants';
import { AuctionTokenEvent } from '../../entities/auction';
import { NumbatSwapAuctionEvent } from '../../entities/auction/numbatnftswap/numbatswap-auction.event';
...
export class StartAuctionEventHandler {
  ...
  private async saveAuction(
    ...
    if (
      marketplace.key === NUMBATNFTSWAP_KEY ||
  ...
  private getEventAndTopics(event: any) {
    if (event.identifier === NumbatNftsSwapAuctionEventEnum.NftSwap) {
      const auctionTokenEvent = new NumbatSwapAuctionEvent(event);
      const topics = auctionTokenEvent.getTopics();
      if (parseInt(topics.auctionType) === NumbatSwapAuctionTypeEnum.Swap) {
