...
import { RedisCacheService } from '@terradharitri/sdk-nestjs';
...
export class ElasticUpdatesEventsService {
  ...
  public async handleRaritiesForNftMintBurnAndUpdateEvents(
    ...
    for (let event of mintEvents) {
      ...
      if (event.identifier === NftEventEnum.DCDTNFTBurn) {
        if (
  ...
  private async isCollectionOfNftsOrSfts(
    ...
    if (
      collectionTypes[collection] === NftTypeEnum.NonFungibleDCDT ||
      collectionTypes[collection] === NftTypeEnum.SemiFungibleDCDT
    ) {
      return true;
    }
    return false;
  ...
  private async isCollectionOfNfts(
    ...
    if (collectionTypes[collection] === NftTypeEnum.NonFungibleDCDT) {
      return true;
    }
    return false;
  }
