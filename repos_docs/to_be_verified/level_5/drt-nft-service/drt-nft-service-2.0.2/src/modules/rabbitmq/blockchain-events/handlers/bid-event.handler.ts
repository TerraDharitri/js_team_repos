...
import { NumbatSwapBidEvent } from '../../entities/auction/numbatnftswap/numbatswap-bid.event';
...
export class BidEventHandler {
  ...
  private getEventAndTopics(event: any, marketplaceKey?: string) {
    if (marketplaceKey && marketplaceKey === NUMBATNFTSWAP_KEY) {
      const bidEvent = new NumbatSwapBidEvent(event);
