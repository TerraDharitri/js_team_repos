...
export class DrtIdentityService {
  ...
  async getProfiles(addresses: string[]): Promise<AccountIdentity[]> {
    const url = `${process.env.NUMBAT_IDENTITY}api/v1/users/multiple`;
    ...
    try {
      ...
      this.logger.error(
        `An error occurred while calling the drt identity service on url ${removeCredentialsFromUrl(
          url,
        )}`,
        {
  ...
  async getProfile(address: string): Promise<AccountIdentity> {
    const url = `${process.env.NUMBAT_IDENTITY}api/v1/users/${address}`;
  ...
  async getFollowersCount(
    ...
    const url = `${process.env.NUMBAT_IDENTITY}api/v1/followers/${address}/count`;
  ...
  async getAcountsByHerotag(searchTerm: string): Promise<any> {
    const url = `${process.env.NUMBAT_IDENTITY}api/v1/herotags/search?criteria=${searchTerm}`;
  ...
  async getAddressByHerotag(herotag: string): Promise<any> {
    const url = `${process.env.NUMBAT_IDENTITY}api/v1/herotags/${herotag}/address`;
