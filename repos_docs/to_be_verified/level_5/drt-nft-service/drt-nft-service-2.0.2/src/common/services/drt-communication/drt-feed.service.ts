...
export class DrtFeedService {
  ...
  async subscribe(identifier: string, authKey?: string): Promise<boolean> {
    if (process.env.ENABLE_FEED_EVENTS === 'true') {
      const url = `${process.env.NUMBAT_FEED}v1/subscriptions`;
      ...
      try {
        ...
        const response = await this.apiService.post(
          ...
          request,
          new ApiSettings({
            apiKey: process.env.NUMBAT_FEED_API_KEY,
        ...
        this.logger.error(
          `An error occurred while calling the drt feed api on url ${removeCredentialsFromUrl(
            url,
          )}`,
          {
  ...
  async addFeed(feed: Feed): Promise<Feed> {
    if (process.env.ENABLE_FEED_EVENTS === 'true') {
      const url = `${process.env.NUMBAT_FEED}v1/feed/item`;

      try {
        const response = await this.apiService.post(
          url,
          feed,
          new ApiSettings({ apiKey: process.env.NUMBAT_FEED_API_KEY }),
        );
        return response.data;
      } catch (error) {
        this.logger.error(
