...
export class DrtStatsService {
  ...
  async getTrending(dimension: 'identifier' | 'collection'): Promise<string[]> {
    const url = `${process.env.NUMBAT_STATS}api/v1/trending/${dimension}`;
    ...
    try {
      ...
      this.logger.error(
        `An error occurred while calling the drt stats service on url ${removeCredentialsFromUrl(
          url,
        )}`,
        {
  ...
  async getNftsViewsCount(identifier: string): Promise<NftViewsCount> {
    const url = `${process.env.NUMBAT_STATS}api/v1/views/nfts/${identifier}/count`;
