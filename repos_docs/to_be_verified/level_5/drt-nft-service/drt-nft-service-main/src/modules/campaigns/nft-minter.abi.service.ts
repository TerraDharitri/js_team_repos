...
export class NftMinterAbiService {
  ...
  async issueToken(request: IssueCampaignRequest): Promise<TransactionNode> {
    ...
    return contract.methodsExplicit
      ...
      .withValue(TokenTransfer.rewaFromBigInteger(drtConfig.issueNftCost))
  ...
  async upgradeNft(ownerAddress: string, request: UpgradeNftRequest): Promise<TransactionNode> {
    ...
    return contract.methodsExplicit
      ...
      .withSingleDCDTNFTTransfer(TokenTransfer.metaDcdtFromBigInteger(collection, parseInt(nonce, 16), new BigNumber(1)))
      .withChainID(drtConfig.chainID)
