...
import { ElasticQuery, QueryType } from '@terradharitri/sdk-nestjs-elastic';
import { ElasticMetricType } from '@terradharitri/sdk-nestjs-monitoring';
...
export class DrtElasticService {
  ...
  async setCustomValue<T>(collection: string, identifier: string, body: any, urlParams: string = null): Promise<void> {
    const uris: string[] = process.env.NUMBAT_ELASTICSEARCH_UPDATE.split(',');
  ...
  async getHoldersCount(): Promise<number> {
    const query = ElasticQuery.create()
      ...
      .withShouldCondition([NftTypeEnum.NonFungibleDCDT, NftTypeEnum.SemiFungibleDCDT].map((type) => QueryType.Match('type', type)))
      .withExtra({
        aggs: {
          unique_addresses: {
            cardinality: {
    ...
    const resultRaw = await this.apiService.post(`${this.url}/accountsdcdt/_search`, query, {
      timeout: 120000,
    });
