...
beforeEach(async () => {
  ...
  deployer = await world.createWallet({
    ...
    kvs: [
      e.kvs.Dcdts([{ id: TOKEN_ID, amount: 1_000, nonce: 1 }]),
    ],
  });
});
...
test('Execute proposal dcdt', async () => {
  ...
  const callData = e.TopBuffer(e.Tuple(
    ...
    ), // arguments to MultiDCDTNFTTransfer function
  ...
  await deployer.callContract({
    ...
    dcdts: [{ id: TOKEN_ID, amount: 1_000, nonce: 1 }],
  });
...
test('Execute proposal upgrade gateway dcdt error', async () => {
  await deployContract();
  ...
  await deployer.callContract({
    ...
    dcdts: [{ id: TOKEN_ID, amount: 500, nonce: 1 }],
  }); // async call actually fails
  ...
  assertAccount(kvs, {
    ...
    kvs: [
      ...
      e.kvs.Dcdts([{ id: TOKEN_ID, amount: 500, nonce: 1 }]), // dcdt still in contract
    ],
  });
  ...
  assertAccount(await deployer.getAccountWithKvs(), {
    balance: 10_000_000_000n,
    kvs: [
      e.kvs.Dcdts([{ id: TOKEN_ID, amount: 1_000, nonce: 1 }]), // got dcdt back
    ],
  });
});
...
test('Withdraw refund token', async () => {
  ...
  assertAccount(await deployer.getAccountWithKvs(), {
    balance: 10_000_000_000n,
    kvs: [
      e.kvs.Dcdts([{ id: TOKEN_ID, amount: 2_000, nonce: 1 }]), // got dcdt back
    ],
  });
  assertAccount(await contract.getAccountWithKvs(), {
    balance: 1_000n,
  ...
  assertAccount(await deployer.getAccountWithKvs(), {
    ...
    kvs: [
      e.kvs.Dcdts([{ id: TOKEN_ID, amount: 2_000, nonce: 1 }]),
    ],
  });
  assertAccount(await contract.getAccountWithKvs(), {
    balance: 0,
