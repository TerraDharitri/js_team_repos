...
describe('Deploy interchain token', () => {
  ...
  test('Only issue dcdt minter no mint', async () => {
    const { baseTokenManagerKvs, computedTokenId } = await deployAndMockTokenManagerInterchainToken();
    ...
    // Assert endpoint to deploy DCDT was called
    assertAccount(await tokenManager.getAccountWithKvs(), {
      balance: 0n,
      hasKvs: [
        ...baseTokenManagerKvs,
...
describe('Deploy remote interchain token', () => {
  ...
  test('REWA no original chain name no minter', async () => {
    await deployAndMockTokenManagerLockUnlock('REWA');
  ...
  test('REWA with original chain name no minter', async () => {
    await deployAndMockTokenManagerLockUnlock('REWA', 'SomeChain');
...
describe('Deploy remote canonical interchain token', () => {
  ...
  test('REWA no original chain name', async () => {
    await deployAndMockTokenManagerLockUnlock('REWA', CHAIN_NAME, CANONICAL_INTERCHAIN_TOKEN_ID);
  ...
  test('REWA with original chain name', async () => {
    await deployAndMockTokenManagerLockUnlock('REWA', 'SomeChain', CANONICAL_INTERCHAIN_TOKEN_ID);
