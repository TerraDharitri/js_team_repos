...
export const setupITSCommands = (program: Command) => {
  ...
  program.command('itsInterchainTransfer')
    ...
    .action(async (tokenIdentifier, amount, gasValue = 1000) => {
      ...
      const result = await wallet.callContract({
        ...
        dcdts: (tokenIdentifier !== 'REWA' ? [{ id: tokenIdentifier, amount: BigInt(amount) }] : []),
      });
  ...
  ********************************/
  ...
  const executePingPongPayload = (wallet: Wallet) => {
    const tokenId = envChain.select<any>(data.knownTokens)['REWA'].tokenId;
  ...
  program.command('itsExpressExecuteWithData').action(async () => {
    ...
    const result = await wallet.callContract({
      ...
      gasLimit: 100_000_000,
      value: BigInt('10000000000000000'), // 0.01 REWA
      funcArgs: [
        e.Str(otherChainName),
  ...
  program.command('itsDeployRemoteInterchainToken')
    .action(async () => {
      ...
      const result = await wallet.callContract({
        ...
        gasLimit: 100_000_000,
        value: BigInt('10000000000000000'), // 0.01 REWA, to pay for cross chain gas
        funcArgs: [
          e.Str(chainName),
          e.TopBuffer(getKeccak256Hash('ITSTT')),
          wallet,
