...
use crate::proxy_its::{DCDT_PROPERTIES_DECIMALS_BUFFER_INDEX, DCDT_PROPERTIES_TOKEN_NAME_INDEX, DCDT_PROPERTIES_TOKEN_TYPE_INDEX};
    ...
    fn deploy_interchain_token(
        ...
        if token_manager.is_zero()
            || self
                ...
                .is_none()
        {
            let rewa_transfer_value = if token_manager.is_zero() {
                require!(
    ...
    fn deploy_remote_token_callback(
        ...
        match result {
            ManagedAsyncCallResult::Ok(values) => {
                ...
                let token_name = vec
                    .get(DCDT_PROPERTIES_TOKEN_NAME_INDEX)
                    .clone_value();
                let token_type = vec.get(DCDT_PROPERTIES_TOKEN_TYPE_INDEX);
                let decimals_buffer_ref =
                    vec.get(DCDT_PROPERTIES_DECIMALS_BUFFER_INDEX);
