...
import { NativeAuthDecoded } from '@terradharitri/sdk-native-auth-server';
...
export class TgBotSocketService {
  ...
  async getAccountState({ address }: NativeAuthDecoded) {
    ...
    const rewaBalance = (await this.getRewaBalance(address)).balance;
    ...
    userStateFromCache.rewaBalance = parseFloat(
      formatAmount({
        input: rewaBalance,
        decimals: 18,
  ...
  private async getRewaBalance(address: string) {
    const response = await axios.get(
