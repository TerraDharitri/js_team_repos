...
describe("test version", () => {
    it("should initialize properly", () => {
        ...
        assert.equal(Version.parse("drtpy v1.2.3").value, "v1.2.3");
        assert.equal(Version.parse("drtpy v1.2.3").vValue, "v1.2.3");
        assert.equal(Version.parse("drtpy v1.2.3").cleanedValue, "1.2.3");
    });

    it("is newer or same as", async () => {
        assert.isTrue(Version.parse("drtpy 1.2.3").isSameAs(Version.parse("v1.2.3")));
        assert.isTrue(Version.parse("drtpy v1.2.3").isSameAs(Version.parse("1.2.3")));
        ...
        assert.isTrue(Version.parse("drtpy v7.0.0").isNewerOrSameAs(Version.parse("drtpy v5.0.0")));
    });
});
