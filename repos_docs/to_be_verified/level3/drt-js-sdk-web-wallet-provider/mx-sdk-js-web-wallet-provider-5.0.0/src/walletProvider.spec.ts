...
describe("test wallet provider", () => {
  ...
  it("login redirects correctly", async () => {
    ...
    assert.equal(
      decodeURI(returnUrl),
      "http://mocked-wallet.com/hook/login?callbackUrl=http://return-to-wallet"
    );

    const returnUrlWithCallback = await walletProvider.login({
      callbackUrl: "http://another-callback",
    });
    assert.equal(
      returnUrlWithCallback,
      "http://mocked-wallet.com/hook/login?callbackUrl=http://another-callback"
    );

    const returnUrlWithToken = await walletProvider.login({
      callbackUrl: "http://another-callback",
      token: "test-token",
    });
    assert.equal(
      returnUrlWithToken,
      "http://mocked-wallet.com/hook/login?token=test-token&callbackUrl=http://another-callback"
    );
  });
  ...
  it("logout redirects correctly", async () => {
    ...
    assert.equal(
      ...
      "http://mocked-wallet.com/hook/logout?callbackUrl=http://return-to-wallet"
    );

    await walletProvider.logout({ callbackUrl: "http://another-callback" });
    assert.equal(
      window.location.href,
      "http://mocked-wallet.com/hook/logout?callbackUrl=http://another-callback"
    );
  });
...
describe("test getTransactionsFromWalletUrl (with relayed)", () => {
  it("gets transactions from wallet url (with relayed)", async () => {
    ...
    assert.deepEqual(signedTransactions, [
      {
        nonce: 1,
        value: "2",
        receiver:
