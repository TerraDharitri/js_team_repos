...
import { UserPublicKey, UserVerifier } from '@terradharitri/sdk-wallet/out';
    ...
    async (context) => {
      try {
        ...
        async function getIdentityOwners(files: { filename: string, raw_url: string }[]): Promise<string[]> {
          ...
          const infoJsonUrl = `https://raw.githubusercontent.com/TerraDharitri/drt-assets/master/${networkPath}identities/${asset}/info.json`;
        ...
        function getApiUrl() {
          switch (network) {
            case 'mainnet':
              return 'https://next-api.dharitri.org';
            case 'devnet':
              return 'https://devnet-api.dharitri.org';
            case 'testnet':
              return 'https://testnet-api.dharitri.org';
          }
        ...
        if (!adminAddress) {
          adminAddress = 'drt1cevsw7mq5uvqymjqzwqvpqtdrhckehwfz99n7praty3y7q2j7yps6facrf';
        }
