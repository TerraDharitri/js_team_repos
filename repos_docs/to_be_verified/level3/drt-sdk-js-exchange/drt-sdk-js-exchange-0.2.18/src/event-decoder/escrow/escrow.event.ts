...
class LockedFunds {
    funds: DcdtTokenPayment[];
    lockedEpoch: number;

    constructor(init: LockedFundsType) {
        this.funds = init.funds.map((token) => new DcdtTokenPayment(token));
    ...
    static fromDecodedAttributes(decodedAttributes: any): LockedFunds {
        return new LockedFunds({
            funds: decodedAttributes.funds.map((token: any) =>
                DcdtTokenPayment.fromDecodedAttributes(token),
            ),
            ...
            new FieldDefinition(
                ...
                new StructType('LockedFunds', [
                    new FieldDefinition(
                        'funds',
                        '',
                        new ListType(DcdtTokenPayment.getStructure()),
                    ),
