import {
  ...
  UserSecretKey,
  UserSigner,
} from "@terradharitri/sdk-wallet/out";
...
export class ClickerLib {
  ...
  private burnAddress =
    "drt1swtk90tzax6p4wg5y82cd43aldgtvenzz4pmnxvg7cv8ma7erf3ssv2q26";
  ...
  /**
   * @description Withdraws all the rewa from the game account address
   *
   * @param {string} addressToWithdraw
   * @return {Promise<IAccountState & {txHash: string}>}
   */
  public async withdrawRewa(
    addressToWithdraw: string
  ): Promise<IAccountState & { txHash: string }> {
    throw new Error("Not implemented");
    // if (this.accountState.rewaBalance <= 0) {
    //   throw new Error("You can not withdraw. Your rewa balance is 0");
    // }
    ...
    //     "",
    //     new BigNumber(this.accountState.rewaBalance),
    //     18
    //   ).toString()
    // )
    ...
    //   (await this.getRewaBalance(this.wallet.publicKey)).nonce
    // );
  ...
  public async withdraw(
    ...
    const transaction = new TransferTransactionsFactory({
      config: factoryConfig,
    }).createTransactionForDCDTTokenTransfer({
      receiver: Address.fromBech32(addressToWithdraw),
      sender: Address.fromBech32(this.accountState.address),
      tokenTransfers,
    });
