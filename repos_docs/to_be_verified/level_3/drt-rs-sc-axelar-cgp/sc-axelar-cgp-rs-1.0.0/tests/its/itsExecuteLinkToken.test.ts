  ...
  const payload = AbiCoder.defaultAbiCoder()
    .encode(
      ...
      [
        ...
        Buffer.from(tokenIdentifier), // message comes from other chain, so the destination token address is set to a valid DCDT identifier
        operator,
      ]
    )
    .substring(2);
...
test('Errors', async () => {
  ...
  // Operator is not a valid DharitrI address
  await user
    .callContract({
      callee: its,
      funcName: 'execute',
    ...
    .assertFail({ code: 4, message: 'Invalid DharitrI address' });
