...
contract Bridge is Initializable, RelayerRole, Pausable {
    ...
    */
    ...
    /**
        ...
        @dev This is for the DharitrI to Ethereum flow
        ...
        @param batchNonceDrt Nonce for the batch. This identifies a batch created on the DharitrI chain that bridges tokens from DharitrI to Ethereum
    ...
    */
    function executeTransfer(
        ...
        uint256 batchNonceDrt,
        bytes[] calldata signatures
    ) public whenNotPaused onlyRelayer {
        require(signatures.length >= quorum, "Not enough signatures to achieve quorum");
        require(executedBatches[batchNonceDrt] == false, "Batch already executed");
        executedBatches[batchNonceDrt] = true;

        _validateQuorum(
            signatures,
            _getHashedDepositData(
                abi.encode(recipients, tokens, amounts, depositNonces, batchNonceDrt, executeTransferAction)
            )
        );
        ...
        CrossTransferStatus storage crossStatus = crossTransferStatuses[batchNonceDrt];
    ...
    /**
        ...
        @param batchNonceDrt Nonce for the batch
     ...
     */
    function getStatusesAfterExecution(uint256 batchNonceDrt) external view returns (DepositStatus[] memory, bool isFinal) {
        CrossTransferStatus memory crossStatus = crossTransferStatuses[batchNonceDrt];
        return (crossStatus.statuses, _isDrtBatchFinal(crossStatus.createdBlockNumber));
    }

    function _isDrtBatchFinal(uint256 createdBlockNumber) private view returns (bool) {
    ...
    function wasBatchExecuted(uint256 batchNonceDrt) external view returns (bool) {
        return executedBatches[batchNonceDrt];
    }
