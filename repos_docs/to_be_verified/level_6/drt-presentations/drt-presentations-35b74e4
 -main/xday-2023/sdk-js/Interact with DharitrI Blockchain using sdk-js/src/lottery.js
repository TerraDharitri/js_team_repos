...
const { Address, SmartContract, TokenTransfer } = require("@terradharitri/sdk-core");
const { ContractAppBase } = require("./shared");
const { CHAIN_ID } = require("./config");

// Contract: https://github.com/TerraDharitri/drt-sdk-rs/blob/master/contracts/examples/lottery-dcdt/src/lottery.rs.
// Documentation: https://github.com/TerraDharitri/drt-sdk-rs/blob/master/contracts/examples/lottery-dcdt/documentation.md.
// Built bytecode: https://github.com/TerraDharitri/drt-reproducible-contract-build-example-sc/releases/download/v0.4.4/lottery-dcdt.wasm.
// For the "lottery-dcdt" example, we will be using the ABI.
async function main() {
    const app = new App();
...
class App extends ContractAppBase {
    async deployContract(cmdObj) {
        ...
        const code = await this.loadContractCode("contracts/lottery-dcdt.wasm");

        // This gets improved in the upcoming version of "sdk-core" (see "drt-sdk-specs").
    ...
    async startLottery(cmdObj) {
        ...
        const abi = await this.loadContractAbi("contracts/lottery-dcdt.abi.json");
        ...
        // "drt-sdk-specs" already define a replacement for the "interactions" API (soon to become available in "sdk-js").
    ...
    async buyTicket(cmdObj) {
        ...
        const transaction = contract.methods.buy_ticket(
            ...
            // Transfer & execute: this gets improved in the upcoming version of "sdk-core" (see "drt-sdk-specs").
            .withSingleDCDTTransfer(new TokenTransfer({ tokenIdentifier, amountAsBigInteger: tokenAmount }))
            .withSender(callerAddress)
            .withGasLimit(5000000)
            .withNonce(callerNonce)
            .withChainID(CHAIN_ID)
