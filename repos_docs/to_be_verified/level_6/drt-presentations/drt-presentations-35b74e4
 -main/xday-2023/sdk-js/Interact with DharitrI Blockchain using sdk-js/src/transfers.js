...
const { Transaction, Address, GasEstimator, TransferTransactionsFactory, TokenTransfer } = require("@terradharitri/sdk-core");
...
async function main() {
    ...
    cli
        .command("transfer-rewa")
        .requiredOption("--receiver <string>")
        .requiredOption("--amount <string>", "in atomic units of the native token (REWA) ")
        .requiredOption("--wallet <string>")
        .action(app.transferREWA.bind(app));

    cli
        .command("transfer-dcdt")
        .requiredOption("--receiver <string>")
        .requiredOption("--token <string>")
        .requiredOption("--amount <string>", "in atomic units of the custom token (must be 1 for NFTs)")
        .requiredOption("--wallet <string>")
        .action(app.transferDCDT.bind(app));
...
class App extends AppBase {
    ...
    // Writing such a helper function will greatly improve in upcoming versions (see the new "drt-sdk-specs").
    createCustomTokenTransferTransaction({
        ...
        if (isFungible) {
            return factory.createDCDTTransfer({
        ...
        return factory.createDCDTNFTTransfer({
            tokenTransfer: tokenTransfer,
            nonce: senderNonce,
            sender: senderAddress,
            receiver: receiverAddress,
    ...
    async getBalances(cmdObj) {
        ...
        console.log(`REWA balance (atomic units): ${account.balance}`);
        ...
        console.log("Non-fungible tokens (including SFT and MetaDCDT, in atomic units):")
