...
import com.durian2.data.model.wallet.SignTransactionInput
...
class WalletInteractor(private val encryptedSharedPreferences: SharedPreferences) {
    ...
    init {
        ...
        identifier?.let{
            ...
            currentPrivateKey = currentHdWallet?.getKeyForCoin(CoinType.DHARITRI)
        }
    }
    ...
    private fun initWallet(
        ...
        val storedKey = StoredKey.importHDWallet(hdWallet.mnemonic(), "", nameByteArray, CoinType.DHARITRI)
        val storedKeyWallet = storedKey.wallet(nameByteArray)
        val address = storedKeyWallet.getAddressForCoin(CoinType.DHARITRI)
        currentHdWallet = storedKeyWallet
        currentPrivateKey = storedKeyWallet.getKeyForCoin(CoinType.DHARITRI)
    ...
    private fun readHdWalletFromStorage(identifier: String): HDWallet {
        ...
        val storedKey = StoredKey.importHDWallet(mnemonic, "", nameByteArray, CoinType.DHARITRI)
