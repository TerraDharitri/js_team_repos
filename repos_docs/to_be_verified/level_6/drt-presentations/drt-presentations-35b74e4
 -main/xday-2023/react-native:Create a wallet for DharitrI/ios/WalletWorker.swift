...
protocol WalletProtocol {
  func createWallet(for password: WalletPassword,
                    and name: WalletName) async throws -> DharitrIWallet
  
  func importWallet(for mnemonic: WalletSecretPhrase,
                    name: WalletName,
                    and password: WalletPassword) async throws -> DharitrIWallet
...
extension WalletWorker: WalletProtocol {
  ...
  func createWallet(for password: WalletPassword,
                    and name: WalletName) throws -> DharitrIWallet {
  ...
  func importWallet(for mnemonic: WalletSecretPhrase,
                    name: WalletName,
                    and password: WalletPassword) throws -> DharitrIWallet {
    ...
    return try getDharitrIWallet(from: wallet)
  }
  ...
  func getPrivateKey(for identifier: WalletIdentifier) throws -> PrivateKey {
    let cacheKey = identifier + String(CoinType.dharitrI.rawValue);
...
private extension WalletWorker {
  ...
  func getDharitrIWallet(from wallet: Wallet) throws -> DharitrIWallet {
    ...
    let accounts = wallet.accounts.compactMap {
      getDharitrIAccount(for: $0.coin, from: hdWallet)
    }
    
    return DharitrIWallet(identifier: wallet.identifier,
                            accounts: accounts,
                            creationDate: wallet.creationDate)
  }
  
  func getDharitrIAccount(for coin: CoinType,
                            from hdWallet: HDWallet) -> DharitrIAccount? {
    let address = hdWallet.getAddressForCoin(coin: coin)
    
    return DharitrIAccount(address: address,
