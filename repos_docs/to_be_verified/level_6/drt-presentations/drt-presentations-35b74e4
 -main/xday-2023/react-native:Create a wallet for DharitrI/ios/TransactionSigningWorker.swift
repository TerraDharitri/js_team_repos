...
extension TransactionSigningWorker: TransactionSigningProtocol {
  ...
  func signTransaction(_ inputTx: TransactionInput,
    ...
    let signedTx: DharitrISigningOutput = AnySigner.sign(input: signerInput,
                                                           coin: .dharitrI)
...
private extension TransactionSigningWorker {
  ...
  func getSignerInput(for inputTx: TransactionInput,
                      and privateKey: PrivateKey) throws -> DharitrISigningInput {
    ...
    let accounts = DharitrIAccounts.with {
    ...
    let action = DharitrIGenericAction.with {
      $0.accounts = accounts
    ...
    let signerInput = DharitrISigningInput.with {
