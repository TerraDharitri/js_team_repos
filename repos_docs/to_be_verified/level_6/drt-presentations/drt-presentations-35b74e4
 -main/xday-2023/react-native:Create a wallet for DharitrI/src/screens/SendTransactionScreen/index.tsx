...
import { generateSignedDharitriTransactions } from 'utils/generateSignedDharitriTransaction';
...
const SendTransactionScreen = () => {
  ...
  const [getDharitriAccount] = useLazyGetDharitriAccountQuery();
  const [sendTransaction] = useSendDharitriTransactionMutation();
  ...
  async function handleSend() {
    onTriggerAction(async () => {
      const dharitriAccount = await getDharitriAccount(
        dharitriAddress
      ).unwrap();
      //TODO check if receiver address is valid before trying to send the transaction
      const signedTransaction = await generateSignedDharitriTransactions({
        amount: values.amount,
        receiver: values.receiver,
        data: values.data,
        sender: dharitriAddress,
        nonce: dharitriAccount?.nonce ?? 1
      });
