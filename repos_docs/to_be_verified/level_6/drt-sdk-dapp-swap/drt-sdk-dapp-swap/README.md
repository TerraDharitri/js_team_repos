...
```typescript
import { SwapAuthorizationProvider } from '@terradharitri/sdk-dapp-swap/components/SwapAuthorizationProvider/SwapAuthorizationProvider';
or;
import { SwapAuthorizationProvider } from '@terradharitri/sdk-dapp-swap/components';
```
...
`graphQLAddress` is a required key that is needed to tell the Apollo Client on what graphQL address it should send the requests. Accepted values are any url string as well as the DharitriX GraphQL service environment based endpoint (provided in `SwapGraphQLAddressEnum`).
