...
import { useGetActiveTransactionsStatus } from '@terradharitri/sdk-dapp/hooks/transactions/useGetActiveTransactionsStatus';
import {
  ApiNetworkProvider,
...
export const Cards = () => {
  ...
  const getContractStakeData = useCallback(() => {
    if (!totalNetworkStake.data || !totalActiveStake.data) {
      ...
      return {
        value: loading ? `... ${network.rewaLabel}` : 'Stake Unknown',
    ...
    return {
      value: `${formatted.nodes} ${network.rewaLabel}`,
  ...
  const getDelegationCap = useCallback(() => {
    if (!contractDetails.data || !totalActiveStake.data) {
      ...
      return {
        value: loading ? `... ${network.rewaLabel}` : 'Cap Unknown',
        percentage: loading ? '...%' : 'Data Unavailable'
      };
    }
    ...
    return {
      value: `${formatted.value} ${network.rewaLabel}`,
  ...
  const cards: CardType[] = [
    ...
    },
    {
      ...
      description: `The delegation cap is the maximum amount of ${network.rewaLabel} your agency can stake from delegators.`,
