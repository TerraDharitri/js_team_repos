...
export class UnlockPanel {
  ...
  render() {
    return (
      <drt-side-panel isOpen={this.isOpen} side={SidePanelSideEnum.RIGHT} onClose={this.handleClose.bind(this)} panelClassName="unlock-panel">
        <drt-unlock-header
        ...
        {!this.isLoggingIn && (
          <div class="body sdk:bg-neutral-400">
            {this.allowedProviders.map(method => (
              <drt-provider-button type={method} onClick={this.handleLogin.bind(this, method)}></drt-provider-button>
            ))}
            <slot></slot>
          </div>
        )}
