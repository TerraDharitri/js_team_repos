...
describe('TransactionAge', () => {
  it('renders with age prop', async () => {
    const page = await newSpecPage({
      ...
      html: '<drt-transaction-age age="2 days ago"></drt-transaction-age>',
    });
  ...
  it('renders without tooltip when not provided', async () => {
    const page = await newSpecPage({
      components: [TransactionAge],
      html: '<drt-transaction-age age="3 hours ago"></drt-transaction-age>',
    });
  ...
  it('renders with tooltip when provided', async () => {
    const page = await newSpecPage({
      components: [TransactionAge],
      html: '<drt-transaction-age age="1 minute ago" tooltip="2023-05-17 10:30:00 UTC"></drt-transaction-age>',
    });
  ...
  it('updates when age prop changes', async () => {
    const page = await newSpecPage({
      components: [TransactionAge],
      html: '<drt-transaction-age age="5 minutes ago"></drt-transaction-age>',
    });
  ...
  it('updates when tooltip prop changes', async () => {
    const page = await newSpecPage({
      components: [TransactionAge],
      html: '<drt-transaction-age age="1 hour ago" tooltip="2023-05-17 09:30:00 UTC"></drt-transaction-age>',
    });
