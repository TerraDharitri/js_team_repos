...
const DCDT_ADDRESS = 'drt1yvesqqqqqqqqqqqqqqqqqqqqqqqqyvesqqqqqqqqqqqqqqqzlllsd5j0s2';
...
export async function issueDcdt(args: IssueDcdtArgs) {
  const txHash = await sendTransaction(new SendTransactionArgs({
    chainSimulatorUrl: args.chainSimulatorUrl,
    sender: args.issuer,
  ...
  const txResponse = await axios.get(`${args.chainSimulatorUrl}/transaction/${txHash}?withResults=true`);
  const dcdtIssueLog = txResponse?.data?.data?.transaction?.logs?.events?.find((event: { identifier: string; }) => event.identifier === 'issue');
...
export async function transferDcdt(args: TransferDcdtArgs) {
  ...
  return await sendTransaction(new SendTransactionArgs({
    ...
    dataField: `DCDTTransfer@${Buffer.from(args.tokenIdentifier).toString('hex')}@${transferValue.toString(16)}`,
    value: '0',
  }));
}
