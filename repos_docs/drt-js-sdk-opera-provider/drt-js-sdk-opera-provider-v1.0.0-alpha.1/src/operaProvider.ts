...
export class OperaProvider {
  ...
  async init(): Promise<boolean> {
    if (window && window.numbat && window.isOpera && !this.initialized) {
      try {
        this.initialized = await window.numbat.init();
      } catch (error) {
  ...
  async login({
    ...
    try {
      this.account.address = await window.numbat.login(token);
    } catch (error: any) {
      throw error;
    }
  ...
  async signTransactions<T extends ITransaction>(
    ...
    try {
      ...
      return await window.numbat.signTransactions(transactions);
    } catch (error) {
      throw error;
    }
  }
  ...
  async signMessage<T extends ISignableMessage>(message: T): Promise<T> {
    try {
      this.ensureConnected();
      return await window.numbat.signMessage(message);
    } catch (error) {
      throw error;
    }
  }
